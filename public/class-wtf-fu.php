...
/**
 *
 * Se incluye el archivo manejador al procesar un audio
 *
 */

define('wtf_fu_AUDIO_COMMAND_HANDLER', plugin_dir_path(__FILE__) . 'includes/JC_commandHandler.php');
...
class Wtf_Fu {
    ...
    private function __construct() {
        ...
        // Se definen acciones manejadoras de peticiones AJAX
        add_action('wp_ajax_wtf_fu_JC', array($this, 'processAudioHandler'));
        add_action('wp_ajax_nopriv_wtf_fu_JC', array($this, 'processAudioHandler'));
    ...
    public function enqueue_scripts() {
        ...
        if (self::wtf_fu_has_shortcode('wtf_fu')) {
            ...
            // Se encola el archivo .js para la adaptación
            $audioSettings_jc_handle = $this->plugin_slug . '-audioFileSettings';

            if (!wp_script_is($audioSettings_jc_handle, 'enqueued')) {
                wp_register_script($audioSettings_jc_handle, plugin_dir_url(__FILE__) . 'assets/js/wtf-fu-audioFileSettings.js', array('jquery', 'wp-ajax-response'), Wtf_Fu::VERSION, true);
                wp_enqueue_script($audioSettings_jc_handle);

                $ret = wp_localize_script($audioSettings_jc_handle, 'WtfFuAjaxVars', 
                        array(
                            'action' => 'wtf_fu_JC',
                            'url' => admin_url('admin-ajax.php'), 
                            'fn' => 'exec_command',
                            'selectedAudioIds'=> array()
                ));
            }
    ...
    function file_upload_JC_shortcode($attr) {
        $shortcode_instance = new Wtf_Fu_Fileupload_JC_Shortcode($attr);
        $content = $shortcode_instance->generate_content();
        return $content;
    }
    ...
    // Funcion del plugin para manejar el procesado de los audios
    function processAudioHandler() {
        Wtf_Fu_Fileupload_JC_Shortcode::processAudioHandler();
