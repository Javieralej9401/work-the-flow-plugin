...
class UploadHandler
{
    ...
    function __construct($options = null, $initialize = true, $error_messages = null) {
        $this->options = array(
            // these three should always be overwritten, 
            // default to something safe though just in case values 
            // dont get through to the handler.
    ...
    protected function set_additional_file_properties($file) {
        ...
        // Add in the wtf_fu directories to the delete url.
    ...
    protected function get_file_objects($iteration_method = 'get_file_object') {
        ...
        /**
         * wtf-fu modificaton.
         * sort scan results in filemtime ASC order.
         * so we match the user selected ordering of files.
         */
        $scan = scandir($upload_dir);
        
        array_multisort(array_map(array($this, 'wtf_fu_sort_files'), $scan), 
                SORT_NUMERIC, SORT_ASC, $scan);       
        
        return array_values(array_filter(array_map(
            array($this, $iteration_method),
            $scan)));
        /* 
         * end wtf-fu modificaton.
         */
...
//        )));
    ...
    protected function validate($uploaded_file, $file, $error, $index) {
        ...
        error_log("post_max_size=$post_max_size , content_length= $content_length");
    ...
    protected function get_scaled_image_file_paths($file_name, $version) {
        ...
        if (!empty($version)) {
            ...
            if (!is_dir($version_dir)) {
                error_log("creating dir version dir $version_dir");
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error,
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            ...
            $upload_dir = $this->get_upload_path();
            if (!is_dir($upload_dir)) {
                                error_log("creating file upload dir $upload_dir");
    ...
    public function get($print_response = true) {
      ...
      // var_dump($userFilesData);
        ...
        if ($file_name) {
            $response = array(
                $this->get_singular_param_name() =>  $userFilesData[0]
            );
        } else {
            $response = array(
    ...
    public function post($print_response = true) {
        ...
        if ($upload && is_array($upload['tmp_name'])) {
            ...
            foreach ($upload['tmp_name'] as $index => $value) {
                $files[] = $this->handle_file_upload(
                    ...
                    $file_name ? $file_name : str_replace(' ', '_', $upload['name'][$index]),
       ...
       //Se insertan los registros de audios subidos en BD
       try {
    ...
    // Funcion que inserta los registros de audios
    public function DBInsertAudioFilesData($audioFilesData = array() ){
        $currentUser = wp_get_current_user();
        $generatedIds = array();
        ...
        foreach ($audioFilesData as $audio) {
            $wpdb->insert( 
                $tableName, 
                 array(
                    'user_id' => $currentUser->ID,
            ...
            array_push($generatedIds, $wpdb->insert_id);
        }
    ...
    public function DBDeleteUserFile($file_name){
       global  $wpdb;
       ...
       $wpdb->delete( 
            $tableName, 
             array( 'user_id' => wp_get_current_user()->ID, 'file_name' => $file_name )
       );
    }
    ...
    public function delete($print_response = true) {
        ...
        foreach($file_names as $file_name) {
            ...
            if ($success) {
                ...
                $this->DBDeleteUserFile($file_name);
            }
